...
func (ctrl *IdentityController) RegisterRoutes(group *gin.RouterGroup) {
	...
	identitiesRoutes := group.Group("/identities")
	{
		...
		identitiesRoutes.POST("", ctrl.CreateIdentityHandler)

		// 更新身份信息 (例如，修改密码)
		// 预期需要认证，允许管理员或用户本人操作 (网关处理认证，服务层或后续逻辑需处理本人或管理员判断)
		identitiesRoutes.PUT("/:identityID", ctrl.UpdateIdentityHandler)

		// 删除身份 (例如，用户解绑登录方式)
		// 预期需要认证，允许管理员或用户本人操作 (同上)
		identitiesRoutes.DELETE("/:identityID", ctrl.DeleteIdentityHandler)
	}

	// 创建 /users 子路由组下的身份相关查询接口。
	// 这些接口通常用于查询某个用户关联的身份信息。
	userSpecificIdentityRoutes := group.Group("/users") // 命名保持不变或简化为 userRoutes
	{
		// 获取指定用户的所有身份类型
		// 预期需要认证，允许管理员或用户本人操作 (同上)

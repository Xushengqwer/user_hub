...
//
// 参数:
//   - userService: 实现了 service.UserService 接口的服务实例。
//   - jwtToken: JWT工具实例。
//   - logger: 日志记录器实例。
//
// 返回:
...
func (ctrl *UserManageController) RegisterRoutes(group *gin.RouterGroup) {
	...
	usersRoutes := group.Group("/users")
	{
		...
		usersRoutes.POST("", ctrl.CreateUserHandler) // 移除中间件

		// 获取用户信息
		// - 场景: 管理员查看用户详情，或用户查看自己的核心信息。
		// - 预期权限: 需要认证，管理员可查看所有用户，普通用户仅能查看自己 (需进行UserID匹配或角色判断)。
		usersRoutes.GET("/:userID", ctrl.GetUserByIDHandler) // 移除中间件

		// 更新用户信息 (角色、状态)
		// - 场景: 管理员修改用户的角色或状态。
		// - 预期权限: 需要认证，且角色为管理员 (Admin)。
		usersRoutes.PUT("/:userID", ctrl.UpdateUserHandler) // 移除中间件

		// 删除用户 (软删除)
		// - 场景: 管理员停用或删除用户账户。
		// - 预期权限: 需要认证，且角色为管理员 (Admin)。
		usersRoutes.DELETE("/:userID", ctrl.DeleteUserHandler) // 移除中间件

		// 拉黑用户 (更新状态)
		// - 场景: 管理员将用户加入黑名单。
		// - 预期权限: 需要认证，且角色为管理员 (Admin)。
		usersRoutes.PUT("/:userID/blacklist", ctrl.BlackUserHandler) // 移除中间件
	}
}

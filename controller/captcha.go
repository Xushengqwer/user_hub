...
// @Success 200 {object} docs.SwaggerAPIEmptyResponse "验证码发送成功（响应体中不包含验证码）"  // 使用 SwaggerAPIEmptyResponse
// @Failure 400 {object} docs.SwaggerAPIErrorResponseString "请求参数无效 (如JSON格式错误、手机号格式不正确)"    // 使用 SwaggerAPIErrorResponseString
// @Failure 500 {object} docs.SwaggerAPIErrorResponseString "系统内部错误 (如短信服务发送失败、Redis存储失败)"      // 使用 SwaggerAPIErrorResponseString
...
func (ctrl *AuthController) RegisterRoutes(group *gin.RouterGroup) {
	...
	{
		// 注册发送短信验证码的接口
		// - 路径: /send-captcha (完整路径如 /api/v1/auth/send-captcha)
		// - 方法: POST
		// - 此接口通常不需要用户认证即可访问。
